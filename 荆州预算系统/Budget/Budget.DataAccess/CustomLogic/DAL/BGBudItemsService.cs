...
namespace BudgetWeb.DAL
{
    public class BGBudItemsService
    {
        ...
        public static DataTable GetBudItemsListByDepid(int Depid)
        {
            ...
            try
            {
                string sqlStr = string.Format("select * from BG_BudItems where DepID={0} and BudSta in ('未提交','退回')", Depid);
                if (Depid == 0)
                {
        ...
        public static DataTable GetApplyReimburByDepID(int DepID,int year, string sta)
        {
            DataTable dt = new DataTable();
            try
            {
                string sqlStr = string.Format("select *,year(BIStaTime) as BIYear from BG_BudItems where DepID={0} and BIYear and BudSta='{1}'", DepID,year, sta);
                if (DepID == 0)
                {
                    sqlStr = string.Format("select *,year(BIStaTime) as BIYear  from BG_BudItems where BudSta='{0}'", sta);
                }
        ...
        internal static DataTable GetBudItemsListByDepid(int Depid, int year)
        {
            DataTable dt = new DataTable();
            try
            {
                string sqlStr = string.Format("select * from BG_BudItems where DepID={0} and BIYear={1} and BudSta in ('未提交','退回')", Depid,year);
                if (Depid == 0)
                {
                    sqlStr = "select *,year(BIStaTime) as BIYear from BG_BudItems where  BudSta in ('未提交','退回')";
                }
