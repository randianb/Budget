...
namespace BudgetWeb.BLL
{
    public class BG_MonPayPlanGenerateLogic
    {
        ...
        public static DataTable GetMonPayTime(string yearMonth, int depid)
        {
            DataTable dt = new DataTable();
            string sqlStr = "select * from  dbo.BG_MonPayPlan where deptid={0} and Convert(varchar(7),MPTime,120)='{1}'";
        ...
        public static bool IsAuditMonPay(string yearMonth, int depid)
        {
            bool flag = false;
            string sqlStr =
                "select count(*) from dbo.BG_MonPayPlanRemark where Convert(varchar(7),MATime,120)='{0}' and DeptID={1} and  MASta in ('审核通过')";
            sqlStr = string.Format(sqlStr, yearMonth, depid);
            if (DBUnity.ExecuteScalar(CommandType.Text, sqlStr, null) == "0")
                flag = false;
            else
        ...
        public static bool IsSubmitMonPay(string yearMonth, int depid)
        {
            bool flag = false;
            string sqlStr =
                "select count(*) from dbo.BG_MonPayPlanRemark where Convert(varchar(7),MATime,120)='{0}' and DeptID={1} and  MASta in ('提交')";
            sqlStr = string.Format(sqlStr, yearMonth, depid);
            if (DBUnity.ExecuteScalar(CommandType.Text, sqlStr, null) == "0")
                flag = false;
            else
        ...
        public static bool IsnotSubmitMonPay(string yearMonth, int depid)
        {
            bool flag = false;
            string sqlStr =
                "select count(*) from dbo.BG_MonPayPlanRemark where Convert(varchar(7),MATime,120)='{0}' and DeptID={1} and  MASta in ('未提交')";
            sqlStr = string.Format(sqlStr, yearMonth, depid);
            if (DBUnity.ExecuteScalar(CommandType.Text, sqlStr, null) == "0")
                flag = false;
            else
        ...
        //修改
        public static bool ISMonlatePay(string yearmonth, int depid)
        {
            bool flag = false;
            string sqlStr =
                "select count(*) from dbo.BG_MonPayPlanRemark where Convert(varchar(7),MATime,120)='{0}' and  MASta='退回' or MASta='审核不通过'   and deptid={1}";
            sqlStr = string.Format(sqlStr, yearmonth, depid);
        ...
        public static int IsMonplanByPIIDDepID(int PIID, int depid, string yearMonth)
        {
            //bool flag = false;
            string sqlStr =
                "select * from  dbo.BG_MonPayPlan where deptid={0} and Convert(varchar(7),MPTime,120)='{1}'and PIID={2}";
        ...
        public static decimal getAllMon(int depid,string YearMonth)
        {
            ...
            try
            {
                string sql = string.Format("select sum([MPFunding]) as Mon from [dbo].[BG_MonPayPlan] where Convert(varchar(7),MPTime,120)='{0}' and DepID={1}", YearMonth,depid);
