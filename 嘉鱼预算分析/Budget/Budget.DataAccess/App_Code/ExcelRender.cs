...
public class ExcelRender
{
    ...
    /// <returns></returns>
    public static MemoryStream RenderToExcelSetHead(DataTable table, string headRow)
    {
    ...
    /// <param name="headRow">设备编号#设备名称#型号</param>
    /// <returns></returns>
    public static void RenderToExcelSetHead_bg_co(DataTable table, string headRow, ISheet sheet)
    {
        IRow headerRow = sheet.CreateRow(0); 
        // handling header.
        //foreach (DataColumn column in table.Columns)
    ...
    /// <param name="headerRowIndex">标题行索引号，如第一行为0</param>
    /// <returns></returns>
    public static DataTable RenderFromExcel1(Stream excelFileStream, int sheetIndex, int rowIndex, int cellNum)
    {
        DataTable table = null;
        ...
        using (excelFileStream)
        {
            using (IWorkbook workbook = new HSSFWorkbook(excelFileStream))
            {
                using (ISheet sheet = workbook.GetSheetAt(sheetIndex))
                {
                    table = RenderFromExcel2(sheet, rowIndex, cellNum);
                }
            }
        }
        return table;
    ...
    /// <param name="headerRowIndex">标题行索引号，如第一行为0</param>
    /// <returns></returns>
    private static DataTable RenderFromExcel2(ISheet sheet, int rowIndex, int cellNum)
    {
        DataTable table = new DataTable();

        for (int k = 0; k < sheet.GetRow(rowIndex).Cells.Count; k++)
        {
        ...
        for (int i = rowIndex; i <= rowCount; i++)
        {
            ...
            if (row != null)
            {
                for (int j = row.FirstCellNum; j < cellNum; j++)
                {
    ...
    /// <param name="headerRowIndex">标题行索引号，如第一行为0</param>
    /// <returns></returns>
    private static DataTable RenderFromExcel(ISheet sheet, int rowIndex, int cellNum)
    {
        DataTable table = new DataTable();
